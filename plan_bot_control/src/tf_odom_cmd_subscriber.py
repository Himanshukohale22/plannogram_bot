...
class pose_node(Node):
    def __init__(self):
        ...
        self.timer_cmd = self.create_timer(0.1, self.cmd_vel_callback)

        self.create_subscription(Odometry, 'bcr_bot/odom', self.odom_callback, 10)
        self.timer_odom = self.create_timer(0.1, self.odom_callback)

        # TF listener
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)
        self.timer_tf = self.create_timer(0.1, self.get_transform)
